    ...
    return function reactImportTransformFactory(context: ts.TransformationContext) {
        return function reactImportTransform(sourceFile: ts.SourceFile) {
            return ts.updateSourceFileNode(
                sourceFile,
                sourceFile.statements.map(updateImportDeclarationIfNeeded).map(updateComponentClassIfNeeded),
            );
        };
    };
}
...
function updateImportDeclarationIfNeeded(statement: ts.Statement) {
    if (
        ...
        return ts.createImportDeclaration(
            undefined,
            undefined,
            ts.createImportClause(undefined, ts.createNamespaceImport(ts.createIdentifier('React'))),
...
function updateComponentClassIfNeeded(statement: ts.Statement) {
    ...
    return ts.createClassDeclaration(
        ...
        [
            ts.updateHeritageClause(firstHeritageClauses, [
                ts.createExpressionWithTypeArguments(
                    expressionWithTypeArguments.typeArguments || [],
                    ts.createPropertyAccess(ts.createIdentifier('React'), typeSymbol),
                ),
            ]),
        ],
        statement.members,
